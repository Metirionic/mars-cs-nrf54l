...
#   - watchdog "CS watchdog" recoveries                         (want 0)
#   - serialized events on "Run serialization"                  (≈ wire rate)
#   - "Serialize backlog: dropped" warnings                     (visible drops,
#     the #173 backpressure valve — nonzero at cadences above ~3.5 events/s,
#     which is the 921600-baud COBS wire limit)
#
...
# blocked in the 289 ms UART TX), starving the #116 liveness watchdog.
#
...
print(f"[{tag}] serialized={serialized} empty_warnings={empty} backlog_reports={backlog} "
      f"watchdog={watchdog} reflector_aborts={refl_aborts} last_proc_counter={last_proc}")
...
if watchdog:
    fail.append(f"{watchdog} watchdog recoveries")
if fail:
    print("RED: " + "; ".join(fail))
    sys.exit(1)
print("GREEN")
