...
int cs_initiator_start(const struct cs_initiator_config * p_config)
{
    ...
    const uint8_t ANTENNA_CONFIG         = get_antenna_config_for_role(BT_CONN_LE_CS_ROLE_INITIATOR);
    const uint8_t PREFERRED_PEER_ANTENNA = get_preferred_peer_antenna_for_role(BT_CONN_LE_CS_ROLE_INITIATOR);
